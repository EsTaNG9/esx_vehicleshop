...
function OpenShopMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			align = 'top-left',
			elements = {
				{label = _U('testdrive_menutext'),  value = 'testdrive'},
				{label = _U('yes'), value = 'yes'},
				{label = _U('no'),  value = 'no'},
		}}, function(data2, menu2)
		if data2.current.value == 'testdrive' then
				--Implemented by EsTaNG#4093, contact me for more info or help
				...
				exports['progressBars']:startUI(40000, "Temporizador do Test Drive")

				ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.TestDrive.Pos, Config.Zones.TestDrive.Heading, function(vehicle)
					
					WaitForVehicleToLoad(vehicleData.model)
					TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
					
					matriculaTeste = 'TESTE ' .. GetRandomNumber(2)
					ESX.ShowNotification(_U('testdrive_notification',Config.TestDriveTime))
					SetVehicleNumberPlateText(vehicle, matriculaTeste)
					
					Citizen.CreateThread(function () 
						local counter = Config.TestDriveTime
						while counter > 0 do 
							counter = counter -1
							Citizen.Wait(1000)
						end
						...
						ESX.ShowNotification(_U('testdrive_finnish'))
						...
						ESX.TriggerServerCallback('esx_vehicleshop:buyVehicle', function(hasEnoughMoney)
							if hasEnoughMoney then
								...
								ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function(vehicle)
									...
									ESX.ShowNotification(_U('vehicle_purchased'))
									local serverIdFromPed = GetPlayerServerId(PlayerId())
									exports['JD_logs']:discord('O **'..GetPlayerName(PlayerId())..'** comprou um **'..vehicleData.name..'** na concessinária normal**\nMatricula: **'..newPlate..'**\nPreço: **'..vehicleData.price..'**', serverIdFromPed, 0, 000, "cardealer_normal")
								end)
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'resell_vehicle' then
					ESX.TriggerServerCallback('esx_vehicleshop:resellVehicle', function(vehicleSold)
						if vehicleSold then
							ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
							local precoCarro = ESX.Math.GroupDigits(CurrentActionData.price)
							ESX.ShowNotification(_U('vehicle_sold_for', CurrentActionData.label, precoCarro))
							local serverIdFromPed = GetPlayerServerId(PlayerId())
							exports['JD_logs']:discord('O **'..GetPlayerName(PlayerId())..'** vendeu um **'..CurrentActionData.label..'** na concessinária normal**\nPreço: **'..precoCarro..'**', serverIdFromPed, 0, 000, "cardealer_normal_vendas")
						else
